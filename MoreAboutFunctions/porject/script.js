...
1. Create a method called 'registerNewAnswer' on the 'poll' object. The method does 2 things:
  ...
  1.2. Based on the input number, update the answers array. For example, if the option is 3, increase the value AT POSITION 3
   of the array by 1. Make sure to check if the input is a number and if the number makes sense 
...
const poll = {
    ...
    registerNewAnswer(){
      const inputFromUser =  Number(prompt(`${this.question}\n${this.options.join('\n')}\n (write option Number)`))
      if(inputFromUser < this.options.length){
        for (let index = 0; index < this.answers.length; index++) {
            if(inputFromUser === index){
                this.answers[index] = this.answers[index] + 1;
            }
        }
      }
      else{
        console.log("invalid Input Number it Should be in Given Range!")
      }
    // typeof inputFromUser === 'number' && inputFromUser < this.answers.length && 
    // this.answers[inputFromUser]++;
